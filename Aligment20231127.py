...
if __name__ == '__main__':
    ...
    for time_window in range(30, 91, 30):#时间窗口多少天
        ...
        serch_condition_times = findUserinTwitter(username, startdatetime,time_window, 'data/dataTwitterPosts.csv'.format(filefold))
        tracking_dict = transTrackingData("data/trackingData.csv".format(filefold))  # 将追踪数据转为以时间为键的方便查找
        for i in range(0, 56, 5):#时间模糊范围多少秒
            ...
            findUserInforByUID(unique_ids, "data/trackingData.csv".format(filefold), "data/dataSinaPosts.csv".format(filefold))
